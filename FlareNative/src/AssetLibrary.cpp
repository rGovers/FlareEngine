...
#define ASSETLIBRARY_BINDING_FUNCTION_TABLE(F) \
    F(uint32_t, FlareEngine.Rendering.UI, Font, GenerateFont, { char* str = mono_string_to_utf8(a_path); const uint32_t ret = AssetLibrary::GenerateFont(str); mono_free(str); return ret; }, MonoString* a_path) \
    F(void, FlareEngine.Rendering.UI, Font, DestroyFont, { AssetLibrary::DestroyFont(a_addr); }, uint32_t a_addr)
...
{
    FLARE_ASSERT_MSG(!a_path.empty(), "AddFont path empty");
...
{
    FLARE_ASSERT_MSG(a_addr < Instance->m_fonts.Size(), "GetFont out of bounds");

    return Instance->m_fonts[a_addr];
}
void AssetLibrary::DestroyFont(uint32_t a_addr)
{
    FLARE_ASSERT_MSG(a_addr < Instance->m_fonts.Size(), "DestroyFont out of bounds");
